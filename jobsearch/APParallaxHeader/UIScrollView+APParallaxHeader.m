...
{
    if(!self.parallaxView) {
        APParallaxView *aPParallaxView = [[APParallaxView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, height)];
        [aPParallaxView setAutoresizingMask:UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleRightMargin|UIViewAutoresizingFlexibleLeftMargin];
        [aPParallaxView.view addSubview:view];
        
        aPParallaxView.scrollView = self;
        aPParallaxView.parallaxHeight = height;
        aPParallaxView.tag = 7777;
        [self addSubview:aPParallaxView];
        
        aPParallaxView.originalTopInset = self.contentInset.top;
        
        UIEdgeInsets newInset = self.contentInset;
        newInset.top = height;
        self.contentInset = newInset;
        
        self.parallaxView = aPParallaxView;
        self.showsParallax = YES;
    }
}
